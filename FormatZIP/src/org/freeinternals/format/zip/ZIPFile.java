...
public class ZIPFile extends FileFormat {
    ...
    /** Central file header signature of File header in Central directory structure. */
    ...
    SortedMap<Long, FileComponent> comp = new TreeMap<Long, FileComponent>();
    ...
    private void parse() throws IOException, FileFormatException {
        ...
        this.comp.put(Long.valueOf(pos_cde), this.cde);
        if (this.cds != null) {
            for (CentralDirectoryStructure cds_item : this.cds) {
                this.comp.put(Long.valueOf(cds_item.getStartPos()), cds_item);
            }
        }
        if (this.lfh != null) {
            for (LocalFileHeader lfh_item : this.lfh) {
                this.comp.put(Long.valueOf(lfh_item.getStartPos()), lfh_item);
            }
        }
    }
    ...
    public void generateTreeNode(DefaultMutableTreeNode root) {
        Iterator iterator = this.comp.keySet().iterator();
        Object key;
        FileComponent value;
        int lastPos = 0, distance;
        while (iterator.hasNext()) {
            key = iterator.next();
            value = this.comp.get(key);
